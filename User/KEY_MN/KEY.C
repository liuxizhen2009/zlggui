...
*@brief
1. 添加了cpu_sr
2、下面这几个函数是硬件相关的，要自己移植
..\Debug\WRT_UCOSII.axf: Error: L6218E: Undefined symbol KeyGetCol (referred from key.o).
..\Debug\WRT_UCOSII.axf: Error: L6218E: Undefined symbol KeyInitPort (referred from key.o).
..\Debug\WRT_UCOSII.axf: Error: L6218E: Undefined symbol KeySelRow (referred from key.o).
*@author
*@param
*@param
*@return
...
#define KEY_MODULE_DEBUG	 1
#if KEY_MODULE_DEBUG
	#define KEY_TLOG(format,...) tfp_printf(format,##__VA_ARGS__)
#else
	#define KEY_TLOG(format,...)
#endif
...
void  KeyInit (void)
{
    ...
    os_err=OSTaskCreate(KeyScanTask, (void *)0, &KeyScanTaskStk[KEY_SCAN_TASK_STK_SIZE], KEY_SCAN_TASK_PRIO);
   	switch(os_err)
		{
			case(OS_ERR_NONE):	break; 
			default:
				ELOG("error:task KeyInit\n");
				while(1);
					break;					 		
		}
}
...
static  void  KeyScanTask (void *data)
{
	...
	KEY_TLOG("KeyScanTask Start Prio=%d\n",OSTCBCur->OSTCBPrio);                                           
...
INT8U  KeyGetCol (void)
{
    return (0);                 /* Complement columns (ones indicate key is pressed)  */
...
void  KeyInitPort (void)
{
                        /* Initialize 82C55: A=OUT, B=IN (COLS), C=OUT (ROWS) */
}
